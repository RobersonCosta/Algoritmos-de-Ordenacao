...
void bubbleSort(Quadrado **quadrados_entrada, int tamanho_entrada){
	bool troca;
	while (1==1){
		troca = false;
		for (unsigned int i = 0; i < tamanho_entrada; i += 1)
		{
			if (i+1<tamanho_entrada){
				if (quadrados_entrada[i+1]->getArea()<quadrados_entrada[i]->getArea()){
					Quadrado *aux = quadrados_entrada[i];
					quadrados_entrada[i] = quadrados_entrada[i+1];
					quadrados_entrada[i+1] = aux;
					troca = true;
				}
			}
		}
